...
def load_properties():
    ...
    if home_directory_name == None:
        ...
        if user_home == None:
            raise Exception(
                "neither EXCHANGE_CLIENT_HOME nor USERPROFILE defined, cannot read client properties and therefore cannot proceed")
...
def cognito_auth(hmac_value, client_id, user_name, password):
    ...
    auth_parameters = {"USERNAME": user_name,
                       "PASSWORD": password, "SECRET_HASH": hmac_value}
    resp = client.initiate_auth(
        ClientId=client_id, AuthFlow="USER_PASSWORD_AUTH", AuthParameters=auth_parameters)
    return resp
